...
def retrieve_and_rerank_experiences(
    ...
    if query_comment and all_candidate_experiences:
        ...
        print(f"    评论处理中，长度为{len(comments)}")
...
def generate_reflection_v2(pr_number, repo: str, before_code: str, review_comment: str,
...
def generate_fix_with_reflection_v2(pr_number, repo: str, before_code: str, review_comment: str,
...
def summarize_experience_v2(pr_number, repo: str, before_code: str, review_comment: str,
                            ...
                            latest_reflection: str, client):
    sys_prompt = (
        f"You are a code repair assistant that writes a reusable Historical Repair Experience(HRE) guideline for OSS project {repo}. "
...
if __name__ == "__main__":
    ...
    for repo in REPO_List:
        DATA_PATH = f"../repo_data/{repo}/{repo}_test.jsonl"
        ...
        RESULT_XLSX = f"./result/{repo}/{repo}_qwen3_8B_test_HRE_outputs_plus_0.xlsx"
        METRIC_FILE = f"./result/{repo}/{repo}_qwen3_8B_test_HRE_metrics_plus_0.txt"
        ...
        for item in tqdm(data, desc="RQ2-HRE processing", unit="item"):
            ...
            ems.append(first_metrics["EM"])
            bleus.append(first_metrics["BLEU"])
            cbleus.append(first_metrics["CodeBLEU"])
            rouges.append(first_metrics["ROUGE-L"])
            edit_progresses.append(first_metrics["EditProgress"])
            ...
            for retry_count in range(MAX_RETRIES):
                if pass_reflection_stop_threshold(best_metrics):
                    ...
                    break
                print(
                    f"第{retry_count}次尝试未达到反思停止阈值(CodeBLEU={best_metrics['CodeBLEU']:.4f}, "
                    f"EditProgress={best_metrics['EditProgress']:.4f})，开始反思重试..."
                )
                retry += 1
                reflection, reflection_time, reflection_input_tokens, reflection_output_tokens, reflection_prompt, \
                    reflection_full_output = generate_reflection_v2(
                ...
                think, new_out, messages, gen_time, input_tokens, output_tokens, gen_full_output = \
                    generate_fix_with_reflection_v2(
                ...
                if not think and not new_out:
                    print(f"  第{retry_count}次反思修复生成失败，保留上一轮结果")
                    continue
            ...
            summarized_experience, summarize_time, summarize_input_tokens, summarize_output_tokens, summarize_prompt, summarize_full_output = summarize_experience_v2(
                pr_number, repo, before_code, review_comment, after_code, best_output,
                best_metrics, latest_reflection, gen_client)
            ...
            if should_update_old_experiences:
                ...
                print("best 结果未达到旧经验更新阈值，仅追加当前 test 样本，不更新旧经验")
